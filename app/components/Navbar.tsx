...
import { Menu, X, Sparkles, ArrowRight, Home } from 'lucide-react';

const navLinks = [
  { href: '/#hero', label: 'Home' }, // Tambahkan slash '/' agar link anchor bekerja dari halaman lain jika diperlukan
  { href: '/#how-it-works', label: 'Cara Kerja' },
  { href: '/#try', label: 'Coba AI' },
...
export default function Navbar() {
  ...
  // 2. DAPATKAN PATH SAAT INI
  const pathname = usePathname();
  
  // 3. LOGIC PENENTU HALAMAN
  // Cek apakah user sedang berada di halaman acne detection
  const isAppPage = pathname === '/acne-detection';
    ...
    <>
      ...
      >
        <div className="relative bg-white/80 backdrop-blur-xl border border-white/50 shadow-lg shadow-sky-100/20 rounded-full px-4 py-3 md:px-6 md:py-3 flex items-center justify-between transition-all duration-300">
          ...
          {/* --- DESKTOP MENU (Tengah) --- */}
          {/* 4. KONDISIONAL RENDERING: Hanya tampil jika BUKAN halaman app */}
          {!isAppPage && (
          ...
          <div className="flex items-center gap-2">
            ...
            {/* 5. TOMBOL BERUBAH SESUAI HALAMAN */}
            {isAppPage ? (
              // Jika di halaman detection: Tampilkan tombol "Kembali ke Home"
              <Link
                href="/"
                className="hidden md:flex items-center gap-2 px-5 py-2 rounded-full bg-gray-100 text-gray-700 text-sm font-semibold hover:bg-gray-200 transition-all duration-300"
              >
                <Home size={14} />
                Kembali ke Home
              </Link>
            ) : (
              // Jika di Landing Page: Tampilkan tombol "Coba AI"
              <Link
                href="/acne-detection"
            ...
            {/* Mobile Menu Button - Hanya tampil di Landing Page jika di App Page menu ditiadakan */}
            {!isAppPage && (
                <button
            ...
            {/* Jika di mobile & di app page, tampilkan tombol home simple icon */}
            {isAppPage && (
                 <Link
                 href="/"
                 className="md:hidden p-2 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-600 transition"
               >
                 <Home size={20} />
               </Link>
            )}
      ...
      {/* --- MOBILE MENU OVERLAY (Hanya Render jika BUKAN App Page) --- */}
      {!isAppPage && (
        <AnimatePresence>
            {isOpen && (
            <motion.div
